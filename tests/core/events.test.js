...
const { emit, on, bus } = require('../../core/events');
...
describe('EventBus', () => {
  ...
  it('delivers to typed and wildcard listeners', async () => {
    const received = [];
    on('test_event', (env) => received.push(['typed', env]));
    on('*', (env) => {
  ...
  it('isolates handler failures — a throwing handler does not break others', async () => {
    ...
    on('fail_test', () => { throw new Error('boom'); });
    on('fail_test', (env) => results.push(env.type));
    on('fail_test', () => results.push('third'));
  ...
  it('isolates async handler failures — a rejecting handler does not break others', async () => {
    ...
    on('async_fail', async () => { throw new Error('async boom'); });
    on('async_fail', (env) => results.push(env.type));
