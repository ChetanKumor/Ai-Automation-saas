...
- **Audit** is inherited only in part. Config revisions and commit evidence make an applied learning artifact attributable to a *proposal* and a *passing validation*; they do **not** make it attributable to a **person** on the operator side. All three admin config writes call `configService.writeTenantConfig` with no `actorUserId`, so `tenant_config_revisions.actor_user_id` lands NULL and the portal renders that NULL as the product name — an operator's edit is indistinguishable from a provisioning write in the owner's own history — and `validation_runs` carries no actor column at all. A platform operator has no representable row to record: that is **F-A017** in `docs/os/state.md`, open, and it blocks the work. Per-actor attribution arrives with the Phase-3 roles named in the two bullets above, not before.
